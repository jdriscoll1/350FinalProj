...
public class Startup {
	...
	public static void A2() throws Exception {
		...
		startup.parse("CREATE REPORTER FREQUENCY myReporter2 NOTIFY ID cli FREQUENCY 3");

		startup.parse("CREATE SENSOR POSITION mySensor2");

		startup.parse("CREATE ACTUATOR LINEAR myActuator2 SENSOR mySensor2 ACCELERATION LEADIN 0.1 LEADOUT -0.2 RELAX 0.3 VELOCITY LIMIT 5 VALUE MIN 1 MAX 20 INITIAL 2 JERK LIMIT 3");

		startup.parse("BUILD NETWORK WITH COMPONENT myActuator2");

		startup.parse("SEND MESSAGE PING");

		startup.parse("SEND MESSAGE ID myActuator2 POSITION REPORT");

		//startup.parse("@CLOCK WAIT FOR 0");
		startup.parse("SEND MESSAGE ID myActuator2 POSITION REQUEST 15");
